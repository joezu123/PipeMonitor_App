...
unsigned char guc_HX_RadarLevel_Addr = 0;
/*******************************************************************************
...
unsigned char* drv_Wait_RecvData_And_CrcCheck(enPara_Type enPaType, unsigned short usRegNum)
{
	...
	//if(pst_MBSystemPara->UsartData.usUsartxRecvDataLen[3] == usRecvDataLength)
	{
		...
		if(func_Get_MBCRC16((unsigned char*)pst_MBSystemPara->UsartData.ucUsart4RecvDataArr,usRecvDataLength) != 0)
		{
            ...
            {
                //pst_MBSystemPara->DeviceRunPara.cModbusNULLCnt111++;
    ...
    else if(enPaType == Type_Float_CDAB) //浮点数四字节，高位在前，低位在后，对换位置
    {
...
unsigned char func_Rts_Control_And_SendData(en_usart_device_t ucDeviceType,unsigned char* ucSendData, unsigned short usDataLen, unsigned short usDelayCnt)
{
    ...
    memset(pst_MBSystemPara->UsartData.ucUsart4RecvDataArr,0,1100);
    ...
    while(pst_MBSystemPara->UsartData.ucUsartxRecvDataFlag[3] == 0)
    {
        ...
        {
            pst_MBSystemPara->DeviceRunPara.cModbusErrCnt++;
            if(pst_MBSystemPara->DeviceRunPara.cModbusErrCnt > 3)
            {
                //pst_MBSystemPara->DeviceRunPara.cModbusNULLCnt++;
                pst_MBSystemPara->DeviceRunPara.cModbusErrCnt = 0;
                pst_MBSystemPara->DeviceRunPara.usDevStatus |= 0x0020; //设置设备异常状态
            }
            //pst_MBSystemPara->DeviceRunPara.usDevStatus |= 0x0020;
...
unsigned char func_Get_Meas_BY_HX_Integrated_Conductivity_Sensor_Value(unsigned char ucDevType, en_usart_device_t ucDeviceType)
{
    ...
    if(ucResult == 0)
    {
        ...
        if(pcRecvData != NULL)
        {
            ...
            //cFloat[2] = pcRecvData[0];
            //cFloat[3] = pcRecvData[1];
            memcpy(cFloat,pcRecvData,4);
            pfMeasValue = &pst_MBSystemPara->DeviceRunPara.esMeasData.es_IntegratedConductivityData.fConductivityValue;
            *pfMeasValue = *((float*)cFloat);
            //cFloat[0] = pcRecvData[6];
            //cFloat[1] = pcRecvData[7];
            //cFloat[2] = pcRecvData[4];
            //cFloat[3] = pcRecvData[5];
            memcpy(cFloat,pcRecvData+4,4);
            pfMeasValue = &pst_MBSystemPara->DeviceRunPara.esMeasData.es_IntegratedConductivityData.fTemperatureValue;
            *pfMeasValue = *((float*)cFloat);
            //cFloat[0] = pcRecvData[10];
            //cFloat[1] = pcRecvData[11];
            //cFloat[2] = pcRecvData[8];
            //cFloat[3] = pcRecvData[9];
             memcpy(cFloat,pcRecvData+8,4);
            pfMeasValue = &pst_MBSystemPara->DeviceRunPara.esMeasData.es_IntegratedConductivityData.fSalinityValue;
            *pfMeasValue = *((float*)cFloat);
            //cFloat[0] = pcRecvData[14];
            //cFloat[1] = pcRecvData[15];
            //cFloat[2] = pcRecvData[12];
            //cFloat[3] = pcRecvData[13];
             memcpy(cFloat,pcRecvData+12,4);
            pfMeasValue = &pst_MBSystemPara->DeviceRunPara.esMeasData.es_IntegratedConductivityData.fTDSValue;
            *pfMeasValue = *((float*)cFloat);
            ucResult = 0;
        }
        else
...
//获取雷达液位传感器设备测量数值
//返回值：0->发送成功; 1->发送失败(超时未接收到设备返回数据)
unsigned char func_Get_Meas_BY_HX_Radar_Level_Sensor_Value(unsigned char ucDevType, en_usart_device_t ucDeviceType)
{
    unsigned char ucSendBuf[10] = {0};
    ...
    if((double)pst_MBSystemPara->DeviceRunPara.esMeasData.esBY_LevelData.fRadarEmptyHeightValue >= 0.01)
    {
        ...
        {
            pst_MBSystemPara->DeviceRunPara.esMeasData.fHXRadarWaterLevelValue = pst_MBSystemPara->DevicePara.fInit_Height - pst_MBSystemPara->DeviceRunPara.esMeasData.esBY_LevelData.fRadarEmptyHeightValue;
        }
    }
    else
    {
        ...
        {
            pst_MBSystemPara->DeviceRunPara.esMeasData.fHXRadarWaterLevelValue = 0.0;
        }
    }
	return ucResult;
}
...
unsigned char func_Get_Meas_HZ_Radar_Ultrasonic_Flow_Sensor_Value(en_usart_device_t ucDeviceType, EMeasSensorType eMeasSensorType)
{
    ...
    for(j=0; j<12; j++)
    {
        ...
        {
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fFlowValue = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fPositiveCumulativeTraffic = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fDischargeArea = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fSurfaceVelocity = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fCross_SectionVelocity = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fWaterLevel = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fEmptyHeight = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fRadarRoll_Angle = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fRadarVertical_Angle = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_Radar_Ultrasonic_FlowData.fRadarInstallHeight = 0.0f;
            ucResult = 1;
            return ucResult;
        }
    }
...
unsigned char func_Get_Meas_HZ_Radar_Level_Sensor_Value(en_usart_device_t ucDeviceType)
{
    ...
    for(j=0; j<4; j++)
    {
        ...
        {
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_LevelData.fRadarEmptyHeightValue = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_LevelData.fRadarRoll_Angle = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_LevelData.fRadarVertical_Angle = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHZ_LevelData.fRadarInstallHeight = 0.0f;
            return 1;
        }
        if(j == 0)
        {
...
//获取HX雷达液位计地址
unsigned char func_Get_HX_Radar_Level_Addr(en_usart_device_t ucDeviceType)
{
    unsigned char ucSendBuf[10] = {0};
	unsigned short usCrc = 0xFFFF;
	unsigned char i = 0;
    ...
    if(ucResult == 0)
    {
        ...
        {
            usValue = *((unsigned short*)pcRecvData);
            ucResult = usValue & 0xFF;
            guc_HX_RadarLevel_Addr = ucResult;
        }
        else
        {
            ucResult = 0;
...
//设置HX雷达液位计地址
unsigned char func_Set_HX_Radar_Level_Addr(en_usart_device_t ucDeviceType)
{
    unsigned char ucSendBuf[10] = {0};
	unsigned short usCrc = 0xFFFF;
	unsigned char i = 0;
...
unsigned char func_Get_Meas_HX_Radar_Ultrasonic_Flow_Sensor_Value(en_usart_device_t ucDeviceType)
{
    ...
    for(j=0; j<10; j++)
    {
        ...
        {
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fCross_SectionVelocity = 0.0f;
            ...
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fUltraFlowVelocity = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fRadarVertical_Angle = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fRadarRoll_Angle = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fEmptyHeight = 0.0f;
    ...
    if(ucResult == 0)
    {
        ...
        if(pcRecvData != NULL)
        {
            ...
            *fMeasValue = *((float*)cTempValue);
            //获取翻滚角
            memcpy(cTempValue,pcRecvData+24,4);
            fMeasValue = &pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fRadarRoll_Angle;
            *fMeasValue = *((float*)cTempValue);
            //获取空高
            memcpy(cTempValue,pcRecvData+36,4);
            fMeasValue = &pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fEmptyHeight;
            *fMeasValue = *((float*)cTempValue);
            //获取瞬时流量
            memcpy(cTempValue,pcRecvData+44,4);
            fMeasValue = &pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fFlowValue;
            *fMeasValue = *((float*)cTempValue);
            //获取累计流量
            memcpy(cTempValue,pcRecvData+48,4);
            fMeasValue = &pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fCumulativeTraffic;
            *fMeasValue = *((float*)cTempValue);
            ucResult = 0;
        }
...
//unsigned char ucTestRecvCnt = 0;
//unsigned char ucTestRecvBuf[20][4] = {{0}};
//获取恒星-截污流量计瞬时流量数据
unsigned char func_Get_Meas_HX_Flow_Sensor_Value(en_usart_device_t ucDeviceType)
{
    ...
    for(j=0; j<6; j++)
    {
        ...
        case 6: //读取设备版本号
            usCMD = 0x1000;
            usRegNum = 1;
            fMeasValue = &fValue;//pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_FlowmeterData.fDeviceVersion;
            break;
        case 7:
            usCMD = 0x1005;
            usRegNum = 1;
        ...
        if(ucResult == 0)
        {
            if(j < 6)
            {
                pcRecvData = (char *)drv_Wait_RecvData_And_CrcCheck(Type_Float_CDAB , 2);
            }
            else
            {
                pcRecvData = (char *)drv_Wait_RecvData_And_CrcCheck(Type_Short , 1);
...
unsigned char func_Get_Meas_HX_WaterQuality_COD_Sensor_Value(en_usart_device_t ucDeviceType, float* fValue)
{
    ...
    for(j=0; j<6; j++)
    {
        ...
        {
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_WaterQuality_CODData.ucSensorStatus = 0;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_WaterQuality_CODData.fCODValue = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_WaterQuality_CODData.fTurbidity = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_WaterQuality_CODData.fCODSignalValue = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_WaterQuality_CODData.fTurbiditySignalValue = 0.0f;
            pst_MBSystemPara->DeviceRunPara.esMeasData.esHX_WaterQuality_CODData.fTemperature = 0.0f;
            return 1;
        }
    }
